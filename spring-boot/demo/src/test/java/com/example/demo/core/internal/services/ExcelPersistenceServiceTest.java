...
class ExcelPersistenceServiceTest {
  ...
  private ExcelPersistenceService persistenceService;

  @BeforeEach
  void setUp() {
    persistenceService = new ExcelPersistenceService(databasePort);
  }
  ...
  void persistValidRows_shouldMapAndPersist() {
    ...
    when(databasePort.persist(anyList(), eq(config), eq(false)))
